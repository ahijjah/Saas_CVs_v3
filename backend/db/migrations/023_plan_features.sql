-- Migration 023: plan_features table + seed for 3 default plans
-- Stores the full 19-feature matrix per plan, editable by super_admin.
...
CREATE TABLE IF NOT EXISTS plan_features (
    ...
    description     TEXT,
    value_type      VARCHAR(20)   NOT NULL CHECK (value_type IN ('boolean', 'number', 'text')),
...
-- RLS: super_admin can do anything; tenants can read all features
ALTER TABLE plan_features ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS plan_features_super_admin ON plan_features;
CREATE POLICY plan_features_super_admin ON plan_features
    USING (current_setting('app.role', true) = 'super_admin');

DROP POLICY IF EXISTS plan_features_tenant_read ON plan_features;
CREATE POLICY plan_features_tenant_read ON plan_features
    FOR SELECT
    USING (current_setting('app.role', true) IN ('tenant_admin', 'recruiter', 'viewer'));

-- Seed 19 features for 3 default plans (trial / starter / professional)
DO $$
DECLARE
    trial_id   UUID;
    starter_id UUID;
...
BEGIN
    ...
    IF trial_id IS NULL OR starter_id IS NULL OR pro_id IS NULL THEN
        RAISE NOTICE 'Default plans (trial/starter/professional) not found — skipping plan_features seed';
        RETURN;
    END IF;

    -- ── Numeric features ────────────────────────────────────────────────────────
    INSERT INTO plan_features (plan_id, feature_key, feature_name, description, value_type, value_number, display_order) VALUES
        (trial_id,   'monthly_cv_processing', 'Monthly CV Processing', 'Number of CVs that can be processed per month',      'number', 20,   1),
        (starter_id, 'monthly_cv_processing', 'Monthly CV Processing', 'Number of CVs that can be processed per month',      'number', 500,  1),
        (pro_id,     'monthly_cv_processing', 'Monthly CV Processing', 'Number of CVs that can be processed per month',      'number', 5000, 1),
        (trial_id,   'active_job_campaigns',  'Active Job Campaigns',  'Maximum number of active job postings at once',      'number', 2,    2),
        (starter_id, 'active_job_campaigns',  'Active Job Campaigns',  'Maximum number of active job postings at once',      'number', 5,    2),
        (pro_id,     'active_job_campaigns',  'Active Job Campaigns',  'Maximum number of active job postings at once',      'number', 25,   2),
        (trial_id,   'number_of_users',       'Number of Users',       'Maximum team members allowed on the platform',       'number', 1,    3),
        (starter_id, 'number_of_users',       'Number of Users',       'Maximum team members allowed on the platform',       'number', 3,    3),
        (pro_id,     'number_of_users',       'Number of Users',       'Maximum team members allowed on the platform',       'number', 15,   3)
    ON CONFLICT (plan_id, feature_key) DO NOTHING;

    -- ── Boolean features: Yes / Yes / Yes ───────────────────────────────────────
    INSERT INTO plan_features (plan_id, feature_key, feature_name, description, value_type, value_boolean, display_order) VALUES
        ...
        (trial_id,   'email_cv_intake',         'Email CV Intake',         'Receive CVs directly via email forwarding or alias',        'boolean', true, 5),
        (starter_id, 'email_cv_intake',         'Email CV Intake',         'Receive CVs directly via email forwarding or alias',        'boolean', true, 5),
        (pro_id,     'email_cv_intake',         'Email CV Intake',         'Receive CVs directly via email forwarding or alias',        'boolean', true, 5),
        ...
        (trial_id,   'duplicate_detection',     'Duplicate Detection',     'Automatic detection and flagging of duplicate submissions',  'boolean', true, 8),
        (starter_id, 'duplicate_detection',     'Duplicate Detection',     'Automatic detection and flagging of duplicate submissions',  'boolean', true, 8),
        (pro_id,     'duplicate_detection',     'Duplicate Detection',     'Automatic detection and flagging of duplicate submissions',  'boolean', true, 8),
    ...
    -- ── Text features ────────────────────────────────────────────────────────────
    INSERT INTO plan_features (plan_id, feature_key, feature_name, description, value_type, value_text, display_order) VALUES
        (trial_id,   'cv_retention_period', 'CV Retention Period', 'How long CV files and data are retained', 'text', '14 Days',   18),
        (starter_id, 'cv_retention_period', 'CV Retention Period', 'How long CV files and data are retained', 'text', '30 Days',   18),
        (pro_id,     'cv_retention_period', 'CV Retention Period', 'How long CV files and data are retained', 'text', '12 Months', 18),
