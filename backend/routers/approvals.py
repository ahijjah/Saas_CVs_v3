...
class InitiateApprovalsRequest(BaseModel):
    """Initiate the approval chain for an application using the tenant's required_approval_stages."""
...
async def list_approvals(
    ...
    return {"approvals": [_serialize_approval(r) for r in rows.mappings()]}
...
async def initiate_approvals(
    ...
    """Create approval records for all required_approval_stages from tenant policy."""
    await set_rls_context(db, current_user.tenant_id, current_user.role)
    await _check_application_access(application_id, current_user, db)

    # Fetch tenant policy stages
    ...
    if not stages:
        raise HTTPException(status_code=422, detail="No required_approval_stages configured for this tenant.")

    # Delete existing pending approvals before re-initiating (idempotent)
    await db.execute(
        text("""
    ...
    for order, stage_name in enumerate(stages):
        approver_id = body.approver_ids.get(stage_name)
        row = await db.execute(
            text("""
